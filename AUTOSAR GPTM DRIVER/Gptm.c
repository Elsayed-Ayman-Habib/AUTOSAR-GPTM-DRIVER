...
#if (GptDevErrorDetect == STD_ON)

#include "Det.h"
/* AUTOSAR Version checking between Det and Gptm Modules */
#if ((DET_AR_MAJOR_VERSION != GPTM_AR_RELEASE_MAJOR_VERSION)\
 || (DET_AR_MINOR_VERSION != GPTM_AR_RELEASE_MINOR_VERSION)\
 || (DET_AR_PATCH_VERSION != GPTM_AR_RELEASE_PATCH_VERSION))
...
void Gpt_init (const Gpt_ConfigType* ConfigPtr)
{
  ...
  if(ConfigPtr ==NULL_PTR)
  {
    Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID,
                   Gpt_GetVersionInfo_SID, GPT_E_PARAM_POINTER);
  }
  else if(Gpt_Status == GPTM_NOT_INITIALIZED)
  {
...
void Gpt_DisableNotification (Gpt_ChannelType Channel)
{
  ...
  #if (GptDevErrorDetect == STD_ON)
  if(Gpt_Status == GPTM_NOT_INITIALIZED)
  {
    Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID, Gpt_DisableNotification_SID,
                    GPT_E_UNINIT); 
  } 
  else if((Channel < 0)||(Channel > GPT_CONFIGURED_CHANNLES ))
  {
      Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID, Gpt_DisableNotification_SID,
                    GPT_E_PARAM_CHANNEL);
  }
  else if (GptNotification[Channel] == NULL_PTR)
  {
       Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID, Gpt_DisableNotification_SID,
                    GPT_E_PARAM_CHANNEL);
  }
...
void Gpt_StartTimer (Gpt_ChannelType Channel,Gpt_ValueType Value)
{
  ...
  else if ((Value < 0)||(Value >(Gpt_Configuration[Channel].GptChannelTickValueMax)*(0xFFFF)))
  {
      Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID, Gpt_StartTimer_SID,
                    GPT_E_PARAM_VALUE);
  }
  else if ((Gpt_Configuration[Channel].Gpt_ChannelState == RUNNING))
  {
      Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID, Gpt_StartTimer_SID,
                    GPT_E_BUSY);
  } 
